import os
from xhtml2pdf import pisa
from django.http import HttpResponse
from io import BytesIO
from django.shortcuts import render
from django.http import FileResponse


def index(request):
    return render(request, 'clubapp/home.html')

def htmlpage(request):
    return render(request, 'htmlfiles/eventreq.html')

from PyPDF2 import PdfWriter, PdfReader
from reportlab.lib.pagesizes import letter
from reportlab.pdfgen import canvas
from django.template.loader import get_template

 

from django.http import HttpResponse
from django.template.loader import get_template
import reportlab

from reportlab.pdfbase import pdfmetrics
from reportlab.pdfbase.ttfonts import TTFont
from django.conf import settings
from xhtml2pdf.files import pisaFileObject


def CreatePDF(request):
    print("aaa")
    PROJECT_DIR = os.path.dirname(os.path.abspath(__file__))
    existing_pdf = os.path.join(PROJECT_DIR, 'club.pdf')
    #pdfmetrics.registerFont(TTFont('arabic', r'C:\Users\emnaa\Fonts\DejaVuSans.ttf'))

    file = BytesIO()

    pdf_reader = PdfReader(open(existing_pdf, "rb"))
    pdf_writer = PdfWriter()
  
    page = pdf_reader.pages[0]

    c = canvas.Canvas(file, pagesize=letter)
    #c.setFont('arabic',32)
    c.showPage()
    c.save()

    template = get_template('htmlfiles/eventreq.html')
    template_content = template.render()  
    pisaFileObject.getNamedFile = lambda self: self.uri
    pdf = pisa.CreatePDF(BytesIO(template_content.encode('utf-8')), file)
    new_pdf = PdfReader(file)

    new_page = new_pdf.pages[0]
    page.merge_page(new_page)
    pdf_writer.add_page(page)
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = 'attachment; filename="hallreq.pdf"'

    output = BytesIO()
    pdf_writer.write(output)
    pdf_data = output.getvalue()
    response.write(pdf_data)
    return response

